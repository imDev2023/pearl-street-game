...
contract FeeRouter {
    ...
    constructor(IERC20Minimal clam_, address prizePool_, address treasury_) {
        require(
            address(clam_) != address(0) && prizePool_ != address(0) && treasury_ != address(0),
            "zero address"
        );
        clam = clam_;
    ...
    function flush() external returns (uint256 toPool, uint256 toTreasury) {
        ...
        if (toPool > 0) clam.safeTransfer(prizePool, toPool);
        if (toTreasury > 0) clam.safeTransfer(treasury, toTreasury);
